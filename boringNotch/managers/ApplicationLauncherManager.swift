...
final class ApplicationLauncherManager: ObservableObject {
    ...
    /// Keeps the launcher current even when it is not open. The short polling
    /// interval also catches applications copied into Applications without launch.
    ...
    func icon(for application: LaunchableApplication) -> NSImage {
        if let cached = iconCache[application.id] {
            return cached
        }

        let loaded = NSWorkspace.shared.icon(forFile: application.url.path)
        iconCache[application.id] = loaded
        return loaded
    }

    func prewarmIcons(limit: Int = 60) async {
        for application in applications.prefix(limit) {
