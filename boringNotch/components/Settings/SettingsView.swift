    ...
    var body: some View {
        HStack(spacing: 0) {
            List(selection: $selectedTab) {
                ...
                Label("Todo", systemImage: "checklist").tag("Todo")
    ...
    var body: some View {
        Form {
            Section {
                ...
                Defaults.Toggle(key: .simulateFullBattery) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Simulate full battery")
                        Text("For testing the 100% layout; does not change the real battery.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
...
struct TodoSettings: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                Button("Clear today's completed tasks") {
                    todoManager.clearCompleted(for: Date())
                }
                .disabled(!showDailyTodo)
                Button("Clear all todos", role: .destructive) {
...
struct Shortcuts: View {
    var body: some View {
        Form {
            ...
            Section {
                KeyboardShortcuts.Recorder("Toggle System Proxy:", name: .toggleClashVergeSystemProxy)
            } header: {
                Text("Tools")
            }
        }
