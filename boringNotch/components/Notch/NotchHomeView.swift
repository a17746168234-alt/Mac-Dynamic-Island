...
struct MusicControlsView: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if Defaults[.enableLyrics] {
                TimelineView(.animation(minimumInterval: 0.25)) { timeline in
                    ...
                    let line: String = {
                        if musicManager.isFetchingLyrics { return String(localized: "Loading lyrics…") }
                        if !musicManager.syncedLyrics.isEmpty {
                            return musicManager.lyricLine(at: currentElapsed)
                        }
                        let trimmed = musicManager.currentLyrics.trimmingCharacters(in: .whitespacesAndNewlines)
                        return trimmed.isEmpty ? String(localized: "No lyrics found") : trimmed.replacingOccurrences(of: "\n", with: " ")
                    }()
    ...
    private var slotToolbar: some View {
        ...
        let hasSidebar = Defaults[.showCalendar] || Defaults[.showDailyTodo]
        let middleSlots = slots.filter { $0 != .previous && $0 != .next }
        if hasSidebar {
            HStack(spacing: 6) {
                if slots.contains(.previous) {
                    slotView(for: .previous).frame(alignment: .center).padding(.leading, 8)
                }
                Spacer(minLength: 0)
                ForEach(Array(middleSlots.enumerated()), id: \.offset) { _, slot in
                    slotView(for: slot).frame(alignment: .center)
                }
                Spacer(minLength: 0)
                if slots.contains(.next) {
                    slotView(for: .next).frame(alignment: .center).padding(.trailing, 8)
                }
            }
...
struct NotchHomeView: View {
    ...
    private var mainContent: some View {
        ...
        let notchWidth = vm.closedNotchSize.width
        let playerWidth: CGFloat = Defaults[.showCalendar]
            ? (Defaults[.showDailyTodo] ? 250 : notchWidth + 79)
            : (Defaults[.showDailyTodo] ? notchWidth + 79 : notchWidth + 252)
        return HStack(alignment: .top, spacing: 8) {
            // 播放器宽度:随开关自适应(修复:关待办后日历贴右、播放器加宽)
            MusicPlayerView(albumArtNamespace: albumArtNamespace)
                .frame(width: playerWidth)

            if Defaults[.showDailyTodo] {
                whiteDivider
                DailyTodoView(width: Defaults[.showCalendar] ? 145 : 165)
            }
