...
struct ContentView: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            VStack(spacing: 0) {
                ...
                mainLayout
                    ...
                    .contextMenu {
                        Button("Settings") {
                            withAnimation(largeNotchTransitionAnimation) {
        ...
        .onChange(of: coordinator.currentView) { oldView, newView in
            ...
            if newView == .settings {
                withAnimation(largeNotchTransitionAnimation) { vm.updateSizeForCurrentView() }
                if oldView.usesLargeNotch {
                    withAnimation(settingsContentAnimation) { presentedView = .settings }
                } else {
                    viewTransitionTask = Task {
                        try? await Task.sleep(for: settingsMountDelay)
                        guard !Task.isCancelled else { return }
                        await MainActor.run { withAnimation(settingsContentAnimation) { presentedView = .settings } }
                    }
                }
            } else if oldView == .settings || presentedView == .settings {
                withAnimation(.easeOut(duration: 0.1)) { presentedView = newView }
                if newView.usesLargeNotch {
                    ...
                    viewTransitionTask = Task {
                        ...
                        guard !Task.isCancelled else { return }
                        await MainActor.run { withAnimation(largeNotchTransitionAnimation) { vm.updateSizeForCurrentView() } }
                    }
                }
            } else {
                withAnimation(largeNotchTransitionAnimation) {
