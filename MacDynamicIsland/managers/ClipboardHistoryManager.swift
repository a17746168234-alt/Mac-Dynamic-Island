...
final class ClipboardHistoryManager: ObservableObject {
    ...
    private static let maximumStoredPayloadSize = 16 * 1_024 * 1_024
    ...
    private func trimToPayloadLimit() {
        var totalSize = 0
        items = items.filter { item in
            ...
            case .image(let data):
                size = data.count
            case .files(let paths):
                size = paths.reduce(0) { $0 + $1.utf8.count }
            }
            guard totalSize + size <= Self.maximumStoredPayloadSize else { return false }
            totalSize += size
            return true
        }
    }
