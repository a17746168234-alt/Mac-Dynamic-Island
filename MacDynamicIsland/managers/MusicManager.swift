...
let defaultImage: NSImage = .init(
    ...
    accessibilityDescription: String(localized: "Album Art")
)!
...
class MusicManager: ObservableObject {
    ...
    @Published var songTitle: String = String(localized: "I'm Handsome")
    @Published var artistName: String = String(localized: "Me")
    @Published var albumArt: NSImage = defaultImage
    @Published var isPlaying = false
    @Published var album: String = String(localized: "Self Love")
    ...
    private var lastArtworkTitle: String = String(localized: "I'm Handsome")
    private var lastArtworkArtist: String = String(localized: "Me")
    private var lastArtworkAlbum: String = String(localized: "Self Love")
                    ...
                    if let result = try await AppleScriptHelper.execute(script),
                       ...
                       !lyricsString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        await self.fetchLyricsFromWeb(
                            ...
                            artist: artist,
                            bundleIdentifier: bundleIdentifier,
                            fallbackPlainLyrics: lyricsString.trimmingCharacters(in: .whitespacesAndNewlines)
                        )
                        return
                    }
                } catch {
    ...
    private func fetchLyricsFromWeb(
        ...
        do {
            if LyricsLookupPolicy.sources(bundleIdentifier: bundleIdentifier).first == .netease,
               let neteaseLyrics = try await Self.fetchNeteaseLyrics(
                   title: title,
                   artist: artist,
                   duration: songDuration
               ) {
                guard songTitle == title, artistName == artist else { return }
                self.currentLyrics = neteaseLyrics
                self.syncedLyrics = self.parseLRC(neteaseLyrics)
                self.isFetchingLyrics = false
                if !self.syncedLyrics.isEmpty { return }
            }

            let exactURL = try LyricsSearchPolicy.searchURL(title: title, artist: artist)
            var candidates = try await Self.fetchLyricsCandidates(from: exactURL)
            var best = LyricsSearchPolicy.bestCandidate(
                in: candidates,
                title: title,
                artist: artist,
                duration: songDuration
            ...
            if best == nil {
                let broadURL = try LyricsSearchPolicy.searchURL(title: title, artist: nil)
                candidates = try await Self.fetchLyricsCandidates(from: broadURL)
                best = LyricsSearchPolicy.bestCandidate(
                    in: candidates,
                    title: title,
                    artist: artist,
                    duration: songDuration
            ...
            guard songTitle == title, artistName == artist else { return }
            if let best {
                let plain = best.plainLyrics?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
                let synced = best.syncedLyrics?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
                ...
                self.currentLyrics = fallbackPlainLyrics
                self.isFetchingLyrics = false
                self.syncedLyrics = []
            }
        } catch {
    ...
    private static func fetchLyricsCandidates(from url: URL) async throws -> [LyricsSearchCandidate] {
        let (data, response) = try await URLSession.shared.data(from: url)
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else { return [] }
        return try JSONDecoder().decode([LyricsSearchCandidate].self, from: data)
    }

    private static func fetchNeteaseLyrics(
        title: String,
        artist: String,
        duration: Double
    ) async throws -> String? {
        var matchedSong: NeteaseSongCandidate?
        for term in NeteaseLyricsPolicy.searchTerms(title: title, artist: artist) {
            let searchRequest = try NeteaseLyricsPolicy.searchRequest(term: term)
            ...
            let songs = try JSONDecoder().decode(NeteaseSearchResponse.self, from: searchData).songs
            if let song = NeteaseLyricsPolicy.bestCandidate(
                in: songs,
                title: title,
                artist: artist,
                duration: duration
        ...
        guard let song = matchedSong else { return nil }

        var lyricsRequest = URLRequest(url: try NeteaseLyricsPolicy.lyricsURL(songID: song.id))
        ...
        guard let lyricsHTTP = lyricsResponse as? HTTPURLResponse, lyricsHTTP.statusCode == 200 else {
            return nil
        }
        let response = try JSONDecoder().decode(NeteaseLyricsResponse.self, from: lyricsData)
        return response.synchronizedLyrics.isEmpty ? nil : response.synchronizedLyrics
    }
