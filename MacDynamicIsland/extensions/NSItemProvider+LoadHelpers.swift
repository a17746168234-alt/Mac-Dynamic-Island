...
extension NSItemProvider {
    ...
    struct LoadedImageRepresentation {
        let data: Data
    ...
    /// Photos and other media libraries often vend an image representation
    /// instead of a stable file URL. This also waits for iCloud-backed assets to
    /// materialize before copying their bytes into shelf-owned temporary storage.
    func loadImageRepresentation() async -> LoadedImageRepresentation? {
        let imageTypes = registeredTypeIdentifiers.compactMap { identifier -> (String, UTType)? in
            guard ShelfDropTypePolicy.isImage(identifier), let type = UTType(identifier) else {
                return nil
            }
            return (identifier, type)
        }
        guard let (typeIdentifier, type) = imageTypes.first else { return nil }
        ...
        return await withCheckedContinuation { continuation in
            loadDataRepresentation(forTypeIdentifier: typeIdentifier) { data, error in
                ...
                var filename = self.suggestedName?.trimmingCharacters(in: .whitespacesAndNewlines)
                if filename?.isEmpty != false {
                    filename = "Photo"
                }
                filename = URL(fileURLWithPath: filename ?? "Photo").lastPathComponent
                if URL(fileURLWithPath: filename ?? "").pathExtension.isEmpty,
                   let fileExtension = type.preferredFilenameExtension {
                    filename = "\(filename ?? "Photo").\(fileExtension)"
                }

                continuation.resume(returning: LoadedImageRepresentation(
                    data: data,
                    suggestedName: filename ?? "Photo.jpeg"
                ))
            }
        }
    }
