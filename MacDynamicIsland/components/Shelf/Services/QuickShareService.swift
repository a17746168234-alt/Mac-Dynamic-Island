...
class QuickShareService: ObservableObject {
    ...
    func showFilePicker(for provider: QuickShareProvider, from view: NSView?) async {
        ...
        panel.title = String(format: String(localized: "Select Files for %@"), provider.localizedName)
        panel.message = String(format: String(localized: "Choose files to share via %@"), provider.localizedName)
    ...
    func shareDroppedFiles(_ providers: [NSItemProvider], using shareProvider: QuickShareProvider, from view: NSView?) async {
        ...
        for provider in providers {
            if let fileURL = await provider.extractFileURL() {
                let resolvedURL = await resolveShelfItemBookmark(for: fileURL) ?? fileURL
                itemsToShare.append(resolvedURL)
            } else if let webURL = await provider.extractURL() {
                itemsToShare.append(webURL)
            } else if foundText == nil, let text = await provider.extractText() {
                foundText = text
            } else if let image = await provider.loadImageRepresentation(),
                      let imageURL = await TemporaryFileStorageService.shared.createTempFile(
                          for: .data(image.data, suggestedName: image.suggestedName)
                      ) {
