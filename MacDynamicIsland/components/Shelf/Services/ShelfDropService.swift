...
struct ShelfDropService {
    ...
    private static func processProvider(_ provider: NSItemProvider) async -> ShelfItem? {
        ...
        if let image = await provider.loadImageRepresentation() {
            if let tempImageURL = await TemporaryFileStorageService.shared.createTempFile(
                for: .data(image.data, suggestedName: image.suggestedName)
            ), let bookmark = createBookmark(for: tempImageURL) {
