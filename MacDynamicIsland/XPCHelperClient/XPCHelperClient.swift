...
final class XPCHelperClient: NSObject {
    ...
    nonisolated func terminateApplication(bundleIdentifier: String) async -> Bool {
        do {
            let service = await MainActor.run {
                ensureRemoteService()
            }
            return try await service.withContinuation { service, continuation in
                service.terminateApplication(bundleIdentifier) { success in
                    continuation.resume(returning: success)
                }
            }
        } catch {
    ...
    struct ClashProxyResult: Sendable {
        let success: Bool
        let errorCode: String?
    }

    nonisolated func setClashVergeSystemProxyEnabled(_ enabled: Bool) async -> ClashProxyResult {
        do {
            let service = await MainActor.run {
                ensureRemoteService()
            }
            return try await service.withContinuation { service, continuation in
                service.setClashVergeSystemProxyEnabled(enabled) { success, errorCode in
                    continuation.resume(returning: ClashProxyResult(success: success, errorCode: errorCode))
                }
            }
        } catch {
            return ClashProxyResult(success: false, errorCode: "xpcUnavailable")
        }
    }

    nonisolated func fetchClashVergeProxyOverview() async -> (String?, String?) {
        do {
            let service = await MainActor.run { ensureRemoteService() }
            return try await service.withContinuation { service, continuation in
                service.fetchClashVergeProxyOverview { json, errorCode in
                    continuation.resume(returning: (json, errorCode))
                }
            }
        } catch {
            return (nil, "xpcUnavailable")
        }
    }

    nonisolated func selectClashVergeNode(_ node: String, group: String) async -> ClashProxyResult {
        do {
            let service = await MainActor.run { ensureRemoteService() }
            return try await service.withContinuation { service, continuation in
                service.selectClashVergeNode(node, forGroup: group) { success, errorCode in
                    continuation.resume(returning: ClashProxyResult(success: success, errorCode: errorCode))
                }
            }
        } catch {
    ...
    nonisolated func testClashVergeNodeDelay(_ node: String) async -> (Int?, String?) {
        do {
            let service = await MainActor.run { ensureRemoteService() }
            return try await service.withContinuation { service, continuation in
                service.testClashVergeNodeDelay(node) { delay, errorCode in
                    continuation.resume(returning: (delay?.intValue, errorCode))
                }
            }
        } catch {
            return (nil, "xpcUnavailable")
